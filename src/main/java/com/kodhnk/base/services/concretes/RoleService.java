...
public class RoleService implements IRoleService {
    ...
    @Override
    public DataResult<Role> addRoleToUser(Long roleId, Long userId) {
        ...
        if (user.isEmpty()) {
            return new ErrorDataResult<>(Response.USER_NOT_FOUND.getMessage(), null, 404);
        }
        user.get().getRoles().add(role.get());
        userRepository.save(user.get());
        return new SuccessDataResult<>(Response.ROLE_ADDED.getMessage(), role.get(), 200);
    }
